...
---Aditional Queries, per readme summary
---Mentor Candidates title count
SELECT COUNT(mentorship_eligibilty.title), mentorship_eligibilty.title
INTO mentor_titles_count
FROM mentorship_eligibilty
GROUP BY mentorship_eligibilty.title
ORDER BY COUNT(mentorship_eligibilty.title) DESC;

--Expanded date range for mentor candidates
SELECT DISTINCT ON (titles.emp_no) 
    employees.emp_no,
    employees.first_name,
    employees.last_name,
...
ON employees.emp_no = titles.emp_no 
WHERE (titles.to_date = '9999-01-01') 
AND (employees.birth_date BETWEEN '1963-01-01' AND '1968-12-31')
ORDER BY titles.emp_no;

---summary of titles for above query:
SELECT COUNT(mentorship_eligibilty_expanded.title), mentorship_eligibilty_expanded.title
INTO mentor_titles_count_expanded
FROM mentorship_eligibilty_expanded
GROUP BY mentorship_eligibilty_expanded.title
ORDER BY COUNT(mentorship_eligibilty_expanded.title) DESC;